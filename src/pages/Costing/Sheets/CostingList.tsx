...
export default function CostingList() {
  ...
  }, [meta.page, meta.limit, search, stageFilter]);
          ...
          <table className="w-full table-auto text-sm">
            <thead>
              <tr className="border-b border-gray-100 dark:border-gray-800">
                {["Costing No", "Style", "Buyer", "Order", "Stage", "Cost/Pc", "Sell/Pc", "Margin%", "Date", "Actions"].map((h) => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={10} className="px-4 py-12 text-center text-gray-400">No costing sheets found</td></tr>
              ) : (
                sheets.map((sheet) => (
                  <tr key={sheet.id} className="border-b border-gray-50 hover:bg-gray-50/50 dark:border-gray-800 dark:hover:bg-white/2">
                    <td className="px-4 py-3">
                      <Link to={`/costing/sheets/${sheet.id}`} className="font-medium text-brand-500 hover:underline">
                        {sheet.costingNo}
                      </Link>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <Link to={`/costing/sheets/${sheet.id}`} className="text-brand-500 hover:text-brand-600" title="Edit">
                          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                        </Link>
                        <button onClick={() => handleDelete(sheet.id)} className="text-red-500 hover:text-red-600" title="Delete">

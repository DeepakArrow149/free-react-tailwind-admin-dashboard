...
import { masterApi, type CompanyMaster } from "../../../api/master";
...
export default function CompanyList() {
  ...
  async function fetchData() {
    setLoading(true);
    try {
      const { data: resp } = await masterApi.listCompanies();
      setItems(resp.data || []);
    } catch (err) {
  ...
  function startEdit(item: CompanyMaster) {
    setEditId(item.id);
    setForm({ code: item.code, name: item.name, address: String(item.address || ""), phone: item.phone || "", email: item.email || "" });
    setShowForm(true);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (editId) {
        await masterApi.updateCompany(editId, form);
      } else {
  ...
  return (
    <>
      <PageMeta title="Companies | ERP TRACK" description="Manage company master" />
      <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/3 p-5 lg:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
          <div>
            <h2 className="text-lg font-semibold text-gray-800 dark:text-white/90">Company Master</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400">{items.length} companie(s)</p>
          </div>
          <button onClick={() => { if (showForm) { resetForm(); } else { setShowForm(true); } }} className="inline-flex items-center gap-2 rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white shadow-theme-xs hover:bg-brand-600">
            {showForm ? "Cancel" : (
              <><svg width="18" height="18" viewBox="0 0 20 20" fill="none"><path d="M10 4.167v11.666M4.167 10h11.666" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" /></svg>Add Company</>
            )}
          </button>
        </div>
          ...
          <form onSubmit={handleSubmit} className="mb-6 rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              <div>
                ...
                <input type="text" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value.toUpperCase() })} placeholder="e.g. COMP01" className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white focus:border-brand-500 focus:ring-brand-500" maxLength={20} />
              </div>
              <div>
                <label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Name *</label>
                <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Company name" className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-white focus:border-brand-500 focus:ring-brand-500" />
              </div>
              <div>

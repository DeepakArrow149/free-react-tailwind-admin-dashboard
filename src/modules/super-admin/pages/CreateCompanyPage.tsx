...
export default function CreateCompanyPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (form.companyCode.length < 2 || form.companyCode.length > 10) {
      setError('Company code must be between 2 and 10 characters.');
      return;
    }
    ...
    try {
      ...
      setSuccess(
        ...
        (res.data.defaultAdmin ? ` Default admin: ${res.data.defaultAdmin}` : '')
      );

      setTimeout(() => navigate('/super-admin/companies'), 2000);
    } catch (err: any) {
      const msg = err?.message || (typeof err === 'string' ? err : 'Failed to create company.');
      setError(msg);
    } finally {
      setLoading(false);
    }
